...
func (SearchTool) Run(ctx context.Context, input map[string]any) (ToolResult, error) {
	query := strings.TrimSpace(stringValue(input["query"]))
	if query == "" {
		return ToolResult{}, errors.New("search query cannot be empty")
	}
...
func runSearchLadder(ctx context.Context, providers []searchProvider, query string, limit int) (ToolResult, error) {
	attempts := []FetchAttempt{}
	var lastError error
	sawEmpty := false
	for _, provider := range providers {
		if provider.key == "" {
			attempts = append(attempts, FetchAttempt{Provider: provider.name, Outcome: "skipped", Detail: "not configured"})
			continue
		}
		started := time.Now()
		...
		if err != nil {
			lastError = err
			attempts = append(attempts, FetchAttempt{Provider: provider.name, Outcome: "error", DurationMS: time.Since(started).Milliseconds(), Detail: conciseError(err)})
			continue
		}
		if len(results) == 0 {
			sawEmpty = true
			attempts = append(attempts, FetchAttempt{Provider: provider.name, Outcome: "empty", DurationMS: time.Since(started).Milliseconds()})
			continue
		}
		attempts = append(attempts, FetchAttempt{Provider: provider.name, Outcome: "ok", DurationMS: time.Since(started).Milliseconds(), Detail: fmt.Sprintf("%d results", len(results))})
		urls := make([]string, 0, len(results))
		for _, result := range results {
			urls = append(urls, result.URL)
		}
		encoded, err := json.Marshal(results)
		return ToolResult{Text: string(encoded), URLs: urls, SeenURLs: urls, Provider: provider.name, Attempts: attempts}, err
	}
	if sawEmpty {
		return ToolResult{Text: "[]", Attempts: attempts}, nil
	}
	if lastError != nil {
		return ToolResult{}, lastError
	}
	return ToolResult{}, errors.New("no search provider configured: set SERPER_API_KEY, EXA_API_KEY, or TAVILY_API_KEY")
}
